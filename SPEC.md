...
6. **Windows libuv shutdown warning under npm/npx (NOT our code).** After `npm run skillenhance ping` (or `npx tsx ...`), an `Assertion failed: !(handle->flags & UV_HANDLE_CLOSING)` warning may print on stderr after our output. Verified diagnosis (2026-07-18):
   - `node --import tsx src/cli.ts ping` → clean (no warning)
   - `npx --no-install tsx src/cli.ts ping` → clean (no warning)
   - `npm run skillenhance -- ping` → warning present
   - The leftover handle belongs to npm/npx's process wrapper layer, not to `src/`. Our CLI exits with code 0 cleanly.
   - Workarounds: use `npx --no-install tsx ...` (skips the install-path that holds the handle), call Node directly (`node --import tsx src/cli.ts ...`), or run via the built `dist/cli.js` after Phase 7 (no tsx dependency).
   - Real fix lives in Node.js / npm upstream (libuv teardown ordering on Windows). Out of our repo's scope.
   - Tracking this for awareness; document the `node --import tsx` direct invocation in README once we publish.
...
| 3. **Rubric + judge** | ✅ | `src/eval/types.ts` (AxisScore record derived from registry), `src/eval/dimensions.ts` (single source of truth — adding a dimension here updates types + Zod + weights + prompt + CLI formatter in one shot), `src/eval/rubric.ts` (5 axes + composite + mastery gate + keep-or-discard, all registry-driven), `src/eval/cases-io.ts` (cases.json read/write + sha256 fingerprint), `src/prompts/judge.ts` (judge prompt template derived from dimensions), `src/eval/judge.ts` (generateText + stripThinking + JSON.extract + Zod.validate — switched from generateObject because that path fails on reasoning-model output), `src/commands/judge.ts` + `skillenhance judge` CLI; 41 new tests; fixtures at `__tests__/fixtures/sample-skill.md` + `cases.json`; real Minimax call returned composite=9.71 across 5 cases × 5 axes; verified adding a 6th dimension (safety, defaultWeight 0) propagates to types + Zod + judge prompt + CLI formatter with no other file changes |
| 4. **Iteration loop** | 🟡 in progress | `src/phase/iterate.ts` (orchestrator), `src/phase/v0.ts` (baseline loader), `src/prompts/mutation.ts` (targeted + broad-rewrite prompt templates), `src/eval/runs-io.ts` (append-only NDJSON runs.jsonl writer + cases sha256 freeze + verify), `src/commands/enhance.ts` (`skillenhance enhance` CLI), bundle writer at `src/phase/output.ts`; uses Phase 3 judge + Phase 2 registry; mutation = generateText + stripThinking + JSON.extract + Zod (same path as judge); mutation temperature 0.3 default; broad-rewrite every 5 iters; early-stop on mastery OR stagnation (3 consecutive rejects) OR round limit; carry forward best only |
